...
这里是为了建立全栈心智模型而做的近似映射，不表示 FastAPI 项目必须机械套用传统 MVC。更严格地说，SQLAlchemy Model 属于持久化模型，Pydantic Schema 属于接口边界的数据契约，Alembic 属于数据库迁移工具；三者职责不同，只是在本课程的“数据侧”共同出现。
...
关于传统服务端 MVC、前后端分离，以及 Schema、ORM、TypeScript 类型之间的完整区别，可继续阅读：[前后端数据流与易混概念](./专题-前后端数据流与易混概念.md)。
