...
DOM 是浏览器对 HTML 的对象表示。Vue 修改响应式状态后，会更新对应 DOM；频繁读取和修改布局相关属性可能触发重复布局，因此业务代码不要手工大量操作 DOM。
...
```text
行内 style > #id > .class / [属性] / :伪类 > 标签 / ::伪元素
```

同等优先级时，后出现的规则覆盖前面的规则；子元素还可能继承父元素的 `color`、`font-family` 等属性。`margin`、`width` 等布局属性通常不继承。

遇到样式不生效时，应在浏览器开发者工具的 Computed/Styles 中确认是哪条规则胜出，不要立即添加 `!important`。
...
```text
block         通常独占一行，可设置宽高
inline        跟随文字排列，宽高通常由内容决定
inline-block  行内排列，同时可设置宽高
```

Flex 和 Grid 会改变直接子元素的布局规则，因此比手工计算 margin 更适合页面布局。
...
| 单位 | 含义 | 常见用途 |
| --- | --- | --- |
| `px` | CSS 像素 | 边框、较固定的小尺寸 |
| `rem` | 相对根元素字号 | 字号、间距、圆角 |
| `%` | 相对包含块 | 流式宽度 |
| `vw` / `vh` | 相对视口 | 全屏区域，使用时注意移动端高度 |
| `fr` | Grid 剩余空间份额 | 网格列宽 |

不要机械地排斥 px，也不要所有尺寸都用 vw。原则是：文字和间距便于缩放，容器能随可用空间变化，并设置合理的 `min-width`、`max-width`。
...
页面默认处于“普通文档流”中：块元素从上到下排列，后面的内容会避开前面的内容。定位方式的区别：

| 值 | 是否保留原位置 | 定位参照 | 常见用途 |
| --- | :---: | --- | --- |
| `static` | 是 | 正常文档流 | 默认布局 |
| `relative` | 是 | 元素原位置 | 小幅偏移、作为绝对定位参照 |
| `absolute` | 否 | 最近的已定位祖先 | 徽标、局部浮层 |
| `fixed` | 否 | 视口 | 全屏遮罩、固定工具栏 |
| `sticky` | 是 | 最近滚动容器 | 吸顶标题 |

`absolute` 和 `fixed` 脱离普通流，滥用后父容器不会自动撑高，也很难适配不同屏幕。
